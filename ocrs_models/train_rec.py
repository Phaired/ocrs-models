...
from .models import RecognitionModel, RecognitionModelV2, RecognitionModelV2Export, RecognitionModelV2Medium
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["v1", "v2", "v2m"],
        default="v1",
        help="Model architecture version: v1 (CRNN/GRU), v2 (CNN+Transformer), v2m (v2 medium - bigger CNN+Transformer)",
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
    ...
    if model_version in ("v2", "v2m"):
        if model_version == "v2m":
            model = RecognitionModelV2Medium(alphabet=DEFAULT_ALPHABET).to(device)
        else:
    ...
    if args.export:
        ...
        if model_version in ("v2", "v2m"):
            # Wrap v2/v2m model for ONNX export to avoid RTen-incompatible
